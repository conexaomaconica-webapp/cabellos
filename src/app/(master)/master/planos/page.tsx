  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Planos Comerciais SaaS</h1>
          ...
          <div key={plan.id} className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4 flex flex-col justify-between shadow-xl">
            <div className="space-y-3">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-3xl font-extrabold text-slate-900 dark:text-white">R$ {plan.monthly_price}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400">/ mês</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">Anual: R$ {plan.yearly_price} | Trial: {plan.trial_days} dias</p>

              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-1.5 text-xs text-slate-700 dark:text-slate-300">
                <p className="font-bold text-slate-200 uppercase tracking-wider text-[11px] mb-2">Limites do Plano:</p>
                <p>• Max Usuários: <strong className="text-slate-900 dark:text-white">{plan.max_users}</strong></p>
                <p>• Max Profissionais: <strong className="text-slate-900 dark:text-white">{plan.max_professionals}</strong></p>
                <p>• Max Clientes: <strong className="text-slate-900 dark:text-white">{plan.max_clients}</strong></p>
              </div>
