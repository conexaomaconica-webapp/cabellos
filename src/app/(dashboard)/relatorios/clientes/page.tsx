  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Análise de Clientes e Retenção</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Recorrência da base, novos atendimentos e clientes em risco de inatividade.</p>
        </div>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-900 dark:text-white text-sm">Clientes Inativos</h3>
          </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(curr.clientes_inativos || []).slice(0, 15).map((client: any) => (
                  <tr key={client.id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white">{client.name}</td>
                    <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400">{client.phone || '—'}</td>
                    <td className="py-2.5 px-3 text-slate-700 dark:text-slate-300">{formatDateBR(client.last_appointment_at)}</td>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-900 dark:text-white text-sm">Clientes Atrasados (Central de Retornos)</h3>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(curr.clientes_atrasados || []).slice(0, 15).map((item: any) => (
                  <tr key={item.alert_id} className="hover:bg-slate-800/40">
                    ...
                    <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400">{item.professional_name || 'Qualquer'}</td>
                    <td className="py-2.5 px-3 text-slate-700 dark:text-slate-300">{formatDateBR(item.due_date)}</td>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <h3 className="font-bold text-slate-900 dark:text-white text-sm">Frequência Média de Retorno por Serviço</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {(curr.frequencias_por_servico || []).map((freq: any, idx: number) => (
            <div key={idx} className="p-3 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1">
              <div className="text-xs font-semibold text-slate-900 dark:text-white">{freq.service_name}</div>
