  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Desempenho por Profissional</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Produção operacional por item executado, ticket médio e apuração de comissões.</p>
        </div>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <h3 className="font-bold text-slate-900 dark:text-white text-sm">Produção Operacional e Comissões</h3>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {professionals.map((p: any) => (
                  <tr key={p.professional_id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white">{p.professional_name}</td>
                    <td className="py-2.5 px-3 text-center font-bold text-slate-200">{p.atendimentos_count}</td>
                    <td className="py-2.5 px-3 text-center text-slate-700 dark:text-slate-300">{p.servicos_count}</td>
                    <td className="py-2.5 px-3 text-center text-slate-700 dark:text-slate-300">{p.clientes_distintos}</td>
                    <td className="py-2.5 px-3 text-right font-bold text-amber-400">{formatCurrency(p.producao_operacional)}</td>
                    <td className="py-2.5 px-3 text-right text-slate-700 dark:text-slate-300">{formatCurrency(p.ticket_medio_operacional)}</td>
