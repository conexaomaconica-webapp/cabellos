  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white">Relatório de Pacotes e Planos</h2>
          <p className="text-xs text-slate-400">Contratos comercializados, utilização de serviços e taxa de renovação.</p>
        </div>
      ...
      {curr.ilimitados_summary && (
        <div className="p-5 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-3">
          <h3 className="font-bold text-white text-sm">Planos Ilimitados — Relação Receita x Consumo</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Valor Recebido do Plano</div>
              <div className="text-lg font-bold text-emerald-400 mt-1">{formatCurrency(curr.ilimitados_summary.valor_recebido)}</div>
            </div>
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Produção Comercial Consumida</div>
              <div className="text-lg font-bold text-amber-400 mt-1">{formatCurrency(curr.ilimitados_summary.producao_consumida)}</div>
            </div>
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Relação Receita x Consumo</div>
              <div className="text-lg font-extrabold text-white mt-1">{formatCurrency(curr.ilimitados_summary.relacao_receita_consumo)}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {curr.creditos_summary && (
        <div className="p-5 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-3">
          <h3 className="font-bold text-white text-sm">Pacotes de Crédito — Utilização de Saldo</h3>
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Créditos Contratados</div>
              <div className="text-lg font-bold text-white mt-1">{curr.creditos_summary.creditos_contratados}</div>
            </div>
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Créditos Utilizados</div>
              <div className="text-lg font-bold text-amber-400 mt-1">{curr.creditos_summary.creditos_utilizados}</div>
            </div>
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Créditos Restantes</div>
              <div className="text-lg font-bold text-indigo-400 mt-1">{curr.creditos_summary.creditos_restantes}</div>
            </div>
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl">
              <div className="text-xs text-slate-400">Taxa de Utilização</div>
      ...
      <div className="p-5 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-4">
        <h3 className="font-bold text-white text-sm">Ranking de Pacotes e Planos Vendidos</h3>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(curr.ranking_produtos || []).map((p: any, idx: number) => (
                  <tr key={idx} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold text-white">{p.package_name}</td>
                    <td className="py-2.5 px-3 text-slate-400 font-medium">{p.package_type}</td>
      ...
      <div className="p-5 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white text-sm">Contratos Próximos do Vencimento (Próximos 30 Dias)</h3>
        </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(curr.planos_vencendo || []).map((item: any) => (
                  <tr key={item.contract_id} className="hover:bg-slate-800/40">
                    ...
                    <td className="py-2.5 px-3 text-slate-300">{formatDateBR(item.expires_at)}</td>
