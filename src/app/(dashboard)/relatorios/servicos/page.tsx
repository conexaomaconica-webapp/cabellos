  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Relatório de Serviços e Categorias</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Desempenho comercial, execução por pacote e participação das categorias.</p>
        </div>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-slate-900 dark:text-white text-sm">Ranking Detalhado de Serviços</h3>
          <span className="text-xs text-slate-500 dark:text-slate-400">Total listados: {ranking.length}</span>
        </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {ranking.map((s: any) => (
                  <tr key={s.service_id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white">{s.service_name}</td>
                    <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400">{s.category_name || 'Sem Categoria'}</td>
                    ...
                    <td className="py-2.5 px-3 text-right text-slate-700 dark:text-slate-300">{formatCurrency(s.ticket_medio)}</td>
      ...
      <div className="p-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        <h3 className="font-bold text-slate-900 dark:text-white text-sm">Produção por Categoria de Serviço</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {(curr.por_categoria || []).map((cat: any, idx: number) => (
            <div key={idx} className="p-4 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
