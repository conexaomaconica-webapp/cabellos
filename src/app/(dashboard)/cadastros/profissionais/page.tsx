    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link href="/cadastros/profissionais/novo">
          <Button variant="default" className="bg-amber-500 text-slate-950 hover:bg-amber-400 font-semibold w-full sm:w-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {professionals.map((prof) => {
            ...
            return (
              <Card
                key={prof.id}
                className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-300 dark:border-slate-700 transition-all flex flex-col justify-between shadow-xl relative overflow-visible group"
              >
                <CardContent className="p-6 space-y-4 flex flex-col items-center text-center">
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <Link href={`/cadastros/profissionais/${prof.id}`}>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-800/80 rounded-xl">
                  ...
                  <div className="relative mt-2">
                    {prof.photo_url ? (
                      <img
                        src={prof.photo_url}
                        alt={prof.name}
                        className="h-24 w-24 rounded-2xl object-cover border-2 border-emerald-500/40 bg-slate-100 dark:bg-slate-950 shadow-xl ring-4 ring-emerald-500/10 hover:scale-[2] hover:z-50 relative transition-transform duration-300 ease-out cursor-pointer hover:shadow-2xl hover:ring-emerald-400"
                      />
                    ) : (
                  ...
                  <div className="space-y-1 w-full">
                    <h3 className="font-bold text-lg text-slate-900 dark:text-white truncate max-w-full">
                      {prof.name}
                    </h3>
                  ...
                  <div className="w-full space-y-2 text-xs text-slate-700 dark:text-slate-300 bg-slate-950/50 p-3 rounded-xl border border-slate-200 dark:border-slate-800/80">
                    {prof.phone && (
                      <div className="flex items-center justify-between gap-2 min-w-0 w-full">
                        <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 shrink-0">
                          <Phone className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /> Contato
                        </span>
                    ...
                    {prof.email && (
                      <div className="flex items-center justify-between gap-2 min-w-0 w-full">
                        <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 shrink-0">
                          <Mail className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /> E-mail
                        </span>
                    ...
                    <div className="space-y-1 w-full">
                      ...
                      <div className="flex flex-wrap justify-center gap-1">
                        {servicesList.map((s, idx) => (
                          <span
                            key={idx}
                            className="inline-block px-2 py-0.5 rounded-md bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[11px] font-medium"
                          >
                            {s}
                          </span>
                        ))}
