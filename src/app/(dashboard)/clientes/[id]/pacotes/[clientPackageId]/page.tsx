  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5 border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <Link href={`/clientes/${clientId}`} className="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-200 border border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-lg">
            ← Ficha do Cliente
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5 bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 space-y-2">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Valor do Contrato</span>
        ...
        <Card className="p-5 bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 space-y-2">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status Financeiro</span>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Pago: R$ {Number(cp.amount_paid).toFixed(2)}</p>
        </Card>

        <Card className="p-5 bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 space-y-2">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Período de Vigência</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6 bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 space-y-4">
          <h2 className="text-base font-semibold text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {(cp.items || []).map((item: any) => {
              ...
              return (
                <div key={item.id} className="p-3 bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
        ...
        <Card className="p-6 bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="space-y-3">
            {(cp.rules || []).map((rule: any) => (
              <div key={rule.id} className="p-3 bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between text-xs text-slate-700 dark:text-slate-300">
        ...
        {(cp.usages || []).length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
            <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
              <thead className="bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 uppercase">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(cp.usages || []).map((u: any) => (
                  <tr key={u.id} className={u.voided_at ? 'opacity-40 line-through' : ''}>
                    ...
                    <td className="p-3 text-slate-500 dark:text-slate-400">{u.professional?.name || '-'}</td>
