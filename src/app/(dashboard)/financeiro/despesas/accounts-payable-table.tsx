  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setIsCreateModalOpen(true)} className="bg-rose-600 hover:bg-rose-500 text-slate-900 dark:text-white font-semibold text-xs gap-1.5 shadow-md">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {payables.map((p) => (
                <tr key={p.id} className="hover:bg-slate-800/30">
                  ...
                  <td className="p-3.5 text-slate-500 dark:text-slate-400">{p.supplier_name || '-'}</td>
                  <td className="p-3.5 text-slate-500 dark:text-slate-400">{p.category?.name || '-'}</td>
                  <td className="p-3.5 text-slate-500 dark:text-slate-400">{new Date(p.due_date).toLocaleDateString('pt-BR')}</td>
                  ...
                  <td className="p-3.5 text-right">
                    {['pending', 'partial'].includes(p.status) && (
                      <Button onClick={() => handleOpenPayModal(p)} className="bg-rose-600 hover:bg-rose-500 text-slate-900 dark:text-white font-bold text-xs gap-1">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 w-full max-w-md space-y-4 text-slate-100 shadow-2xl">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 dark:text-slate-300">Descrição da Despesa *</label>
                <Input
                  value={description}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="font-semibold text-slate-700 dark:text-slate-300">Fornecedor / Favorecido</label>
                  <Input
                    value={supplierName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="font-semibold text-slate-700 dark:text-slate-300">Valor Total (R$) *</label>
                  <Input
                    type="number"
                    step="0.01"
                    min="0.01"
                    ...
                    className="bg-slate-100 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-rose-400 font-bold text-base"
                    required
                  />
                </div>

                <div className="space-y-1">
                  <label className="font-semibold text-slate-700 dark:text-slate-300">Data de Vencimento *</label>
                  <Input
                    type="date"
                    value={dueDate}
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                <Button type="button" variant="outline" onClick={() => setIsCreateModalOpen(false)} className="border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
                  Cancelar
                </Button>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 w-full max-w-md space-y-4 text-slate-100 shadow-2xl">
            ...
            <div className="p-3 bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-xs space-y-1">
              <p className="text-slate-500 dark:text-slate-400">Despesa: <span className="text-slate-100 font-semibold">{selectedPayable.description}</span></p>
              <p className="text-slate-500 dark:text-slate-400">Saldo Restante a Pagar: <span className="text-rose-400 font-bold">R$ {Number(selectedPayable.remaining_amount).toFixed(2)}</span></p>
            </div>
            ...
            <form onSubmit={handlePaySubmit} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 dark:text-slate-300">Valor Pago (R$) *</label>
                <Input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                <Button type="button" variant="outline" onClick={() => setSelectedPayable(null)} className="border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
                  Cancelar
                </Button>
                <Button type="submit" disabled={isPending} className="bg-rose-600 hover:bg-rose-500 text-slate-900 dark:text-white font-bold px-4">
