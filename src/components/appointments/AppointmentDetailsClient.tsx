  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/atendimentos" className="inline-flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div>
                <p className="font-bold text-base text-slate-900 dark:text-white">{appointment.client?.name}</p>
              ...
              <Link href={`/clientes/${appointment.client_id}`}>
                <Button variant="outline" size="sm" className="border-slate-300 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300">
                  Ver Ficha
                </Button>
              </Link>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {appointment.services?.map((item) => (
                <div
                  key={item.id}
                  className="p-3 rounded-xl bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs"
                >
                  <div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {appointment.payments && appointment.payments.length > 0 ? (
                ...
                <p className="text-xs text-slate-500 italic p-3 rounded-xl bg-slate-950/60 border border-slate-200 dark:border-slate-800">
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-in fade-in-50">
          <Card className="w-full max-w-md bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white shadow-2xl">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Motivo do Cancelamento</label>
                <textarea
                  value={reason}
                  onChange={(e) => setReason(e.target.value)}
                  placeholder="Ex: Desistência do cliente, erro no lançamento..."
                  className="w-full rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 p-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500"
                  rows={3}
                />
              </div>

              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="secondary" onClick={() => setIsCancelModalOpen(false)} className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                  Voltar
                </Button>
                <Button
                  type="button"
