...
export function SimpleLineChart({ title, data }: SimpleLineChartProps) {
  if (!data || data.length === 0) {
    return (
      <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl text-center text-slate-400 text-sm">
          ...
          <tbody className="divide-y divide-slate-800/60">
            {data.map((item, idx) => (
              <tr key={idx} className="hover:bg-slate-800/40">
                <td className="py-2 px-3 font-medium text-slate-200">{item.date}</td>
                <td className="py-2 px-3 text-right text-amber-400 font-medium">{formatCurrency(item.producao)}</td>
                <td className="py-2 px-3 text-right text-emerald-400 font-medium">{formatCurrency(item.recebimentos)}</td>
                <td className="py-2 px-3 text-right text-rose-400 font-medium">{formatCurrency(item.despesas)}</td>
                <td className="py-2 px-3 text-right font-bold text-white">{formatCurrency(item.resultado_caixa)}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          return (
            <div key={idx} className="space-y-1">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-bold text-amber-400">{valueFormatter(item.value)}</span>
              </div>
