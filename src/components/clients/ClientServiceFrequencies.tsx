          ...
          {frequencies.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {frequencies.map((freq) => (
                <div key={freq.id} className="p-4 rounded-xl bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs p-3 rounded-lg bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80">
                    <div>
        ...
        <CardContent className="p-6">
          {contacts.length === 0 ? (
            ...
            <div className="space-y-3">
              {contacts.map((c) => (
                <div key={c.id} className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-1.5 text-xs">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-[10px] uppercase text-emerald-400 border-emerald-500/30">
                        {c.contact_type}
                      </Badge>
                      <span className="text-slate-500 dark:text-slate-400 font-mono">{formatDate(c.contacted_at)}</span>
                    </div>
                    <Badge
                  ...
                  <p className="text-slate-200 font-mono bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800/80">
                    &quot;{c.message_content}&quot;
                  </p>
                  {c.notes && <p className="text-[11px] text-slate-500 dark:text-slate-400 italic">Obs: {c.notes}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-slate-900 dark:text-white space-y-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-slate-200 dark:border-slate-800">
              <Button variant="ghost" onClick={() => setSelectedFreq(null)} className="text-slate-500 dark:text-slate-400 text-xs">
                Cancelar
              </Button>
